...
export async function GET(req: NextRequest) {
  ...
  const membros = await sql`SELECT u.id, u.name, u.email, fm.role FROM family_members fm JOIN users u ON u.id = fm.user_id WHERE fm.family_id = ${familia.id} ORDER BY fm.joined_at`
